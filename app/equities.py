...
from util.utility import RunScreener, py_mail, send_mail
import pandas as pd

stocks = 'etsy.us'  # ['SQ.us', 'MSFT', 'AMZN', 'WB']
...
def generate_stock_alerts():
    ...
    se = StooqExchange()  # StockExchange()
    to_date = dt.datetime.now(local_tz)
    from_date = to_date - dt.timedelta(days=30)
    result_str = ''

    stock_list = [[stocks, se.get_data(stocks, from_date, to_date)]]
